...
namespace ppconsul { namespace netlib {
    ...
    std::tuple<http::Status, ResponseHeaders, std::string> HttpClient::get(const std::string& path)
    {
        boost::network::http::client::request request(makeUrl(path));
        auto response = m_client.get(request);
        return std::make_tuple(getStatus(response), getHeaders(response), body(response));
    }

    std::pair<http::Status, std::string> HttpClient::put(const std::string& path, const std::string& data)
    {
        boost::network::http::client::request request(makeUrl(path));
    ...
    std::pair<http::Status, std::string> HttpClient::del(const std::string& path)
    {
        boost::network::http::client::request request(makeUrl(path));
